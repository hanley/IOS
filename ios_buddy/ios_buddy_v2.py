...
@st.cache_resource
def load_model():
    with st.spinner("Downloading DistilBERT Embeddings Model locally....please be patient"):
        embedding_model = HuggingFaceEmbeddings(model_name="distilbert-base-uncased", model_kwargs={"device": "cpu"})
...
class ChatWithIOSXE:
    ...
    def chat(self, question):
        ...
        ai_response = f"Cisco IOS: {answer}"
...
def run_pyats_job():
    os.system("pyats run job ios_buddy_job.py")

def update_testbed_file(ip, username, password):
    # Load the existing YAML file
    with open('testbed.yaml', 'r') as file:
        testbed = yaml.safe_load(file)

    # Update the YAML structure with new values
    if 'devices' in testbed:
        for device_name, device in testbed['devices'].items():
            if 'connections' in device and 'cli' in device['connections']:
                device['connections']['cli']['ip'] = ip
                if 'credentials' in device and 'default' in device['credentials']:
    ...
    with open('testbed.yaml', 'w') as file:
        yaml.safe_dump(testbed, file, default_flow_style=False)

def page_pyats_job():
    
    ios_ip = st.text_input('Please Enter the device IP address', '')
    ios_user = st.text_input('Please Enter the username', '')
    ios_password = st.text_input('Please Enter the password', type='password')        
    
    if st.button('Update Testbed'):    
        # Update the testbed.yaml file
        update_testbed_file(ios_ip, ios_user, ios_password)
        
        st.success('testbed file updated successfully!')
